...
time-mcp-server is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i time-mcp-server:latest
   ```
...
kubectl get mcpserver time-mcp-server
```