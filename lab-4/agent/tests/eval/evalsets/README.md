...
`basic.evalset.json` is the canonical evalset for the Public GitHub Documentation Agent. The cases are curated against the three rubrics in `../eval_config.json`: repo-specific questions (`repo_routing_envoy`, `repo_overview_kubernetes`, `cross_repo_comparison`) must be grounded in a DeepWiki MCP tool call and cite the `owner/repo` identifier; off-topic and private-repo prompts (`off_topic_capital`, `private_repo_refusal`) must be politely refused without contacting DeepWiki. To extend the evalset, copy a case, add a new `eval_id`, and pick a prompt that stresses one of the rubrics (a new repo, a multi-repo synthesis, or a fresh refusal scenario); the schema below applies unchanged.
